...
Att_analog_in& Att_analog_in::range( int toLow, int toHigh ) {
  _toLow = toLow;
  _toHigh = toHigh;
  return *this;
}

Att_analog_in& Att_analog_in::onChange( Machine& machine, int event /* = 0 */ ) {
  _onchange.set( &machine, event );
  return *this;
}

Att_analog_in& Att_analog_in::onChange( TinyMachine& machine, int event /* = 0 */ ) {
  _onchange.set( &machine, event );
  return *this;
}

Att_analog_in& Att_analog_in::onChange( atm_analog_in_cb_t callback, int idx /* = 0 */ ) {
  _onchange.set( (atm_cb_t)callback, idx );
  return *this;
}

#ifndef TINYMACHINE
Att_analog_in& Att_analog_in::onChange( const char* label, int event /* = 0 */ ) {
