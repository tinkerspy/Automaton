...
void Atm_bit::comm( Atm_Bit_Comm & c, state_t st ) {
...
Att_bit & Att_bit::onFlip( bool st, stepcb_t callback )
{
  short idx = st ? 0 : 1;
  _comm[idx]._mode = MODE_CALLBACK;
  _comm[idx]._callback = callback;
...
Att_bit & Att_bit::onFlip( bool st, Machine & machine, state_t event /* = 0 */ )
{
  short idx = st ? 0 : 1;
  _comm[idx]._mode = MODE_MACHINE;
  _comm[idx]._client_machine = &machine;
...
Att_bit & Att_bit::onFlip( bool st, TinyMachine & machine, state_t event /* = 0 */ )
{
  short idx = st ? 0 : 1;
  _comm[idx]._mode = MODE_TMACHINE;
  _comm[idx]._client_tmachine = &machine;
...
Att_bit & Att_bit::onInput( bool st, stepcb_t callback )
{
  short idx = st ? 2 : 3;
  _comm[idx]._mode = MODE_CALLBACK;
  _comm[idx]._callback = callback;
...
void Att_bit::comm( Atm_Bit_Comm & c, state_t st ) {
