...
Atm_digital_in& Atm_digital_in::begin( int attached_pin, int debounce /* = 20 */, bool activeLow /* = false */, bool pullUp /* = false */ ) {
  // clang-format off
...
Atm_digital_in& Atm_digital_in::onFlip( bool st, Machine& machine, int event /* = 0 */ ) {
  _connection[st ? 1 : 0].set( &machine, event );
  return *this;
}

Atm_digital_in& Atm_digital_in::onFlip( bool st, TinyMachine& machine, int event /* = 0 */ ) {
  _connection[st ? 1 : 0].set( &machine, event );
  return *this;
}

#ifndef TINYMACHINE
Atm_digital_in& Atm_digital_in::onFlip( bool st, const char* label, int event /* = 0 */ ) {
  _connection[st ? 1 : 0].set( label, event );
  return *this;
}
#endif

int Atm_digital_in::event( int id ) {
  switch ( id ) {
    case EVT_TIMER:
...
void Atm_digital_in::action( int id ) {
  switch ( id ) {
    case ACT_HIGH:
