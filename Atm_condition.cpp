...
bool Atm_condition::eval_one( uint8_t idx ) {
  ...
  switch ( _operand[idx].relOp() ) {
    case atm_connector::REL_EQ  : return _operand[idx].pull( factory ) == _operand[idx].event;
    case atm_connector::REL_NEQ : return _operand[idx].pull( factory ) != _operand[idx].event;
    case atm_connector::REL_LT  : return _operand[idx].pull( factory ) <  _operand[idx].event;
    case atm_connector::REL_GT  : return _operand[idx].pull( factory ) >  _operand[idx].event;
    case atm_connector::REL_LTE : return _operand[idx].pull( factory ) <= _operand[idx].event;
    case atm_connector::REL_GTE : return _operand[idx].pull( factory ) >= _operand[idx].event;      
  }       
  return false;
}
...
void Atm_condition::action( int id ) {
  switch ( id ) {
    ...
    case ACT_ON :
      if ( _last_state != -1 )
         _connection[ _last_state == current ? 2 : 0 ].push( factory );
      _last_state = current;
      return;
   }
}
