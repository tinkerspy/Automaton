...
class Atm_analog_in : public MACHINE {
 public:
  ...
  Atm_analog_in& onChange( Machine& machine, int event = 0 );
  Atm_analog_in& onChange( TinyMachine& machine, int event = 0 );
  Atm_analog_in& onChange( atm_analog_in_cb_t callback, int idx = 0 );
  Atm_analog_in& onChange( const char* label, int event = 0 );
